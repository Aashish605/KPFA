    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent className='mt-1 w-[min(93vw,800px)]' align={align}>
        {navigationData.map(navItem => {
          if (navItem.href) {
            const isHashLink = navItem.href.startsWith('#')
            const sectionId = isHashLink ? navItem.href.replace('#', '') : ''
            const isActive = isHashLink && activeSection === sectionId && activeSection !== ''
